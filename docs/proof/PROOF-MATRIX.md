...
or changing any proof gate. Four public runtime exports and eighteen public types bind an exact
M04-T06 state lifetime to a trusted complete same-Bundle surface inventory, evaluate `when` before
type-specific payload observation, and share one bounded detached token session across a true
guard and its payload. State set/toggle delegate complete schema-safe writes to M04-T06; toggle
accepts only an exact boolean. Navigation authorizes the local target before parameters, emits
exact `ENTRY_NOT_FOUND`, `NAVIGATION_DENIED`, and redacted `ADAPTER_FAILURE` outcomes, and
terminally disposes the old executor and local state on success, including same-surface success.
Every hostile reflection and callback boundary rechecks both lifetime and exact state authority.
Forty-two focused package tests, eleven compiler-negative cases, and nineteen root proof/mutation
tests enforce these semantics, API containment, portability, five direct trace assignments, and
sixteen task-owned files. Multi-action turns, settlement dispatch, component/event bridges,
...
changing any proof gate. Four public runtime exports and sixteen public types bind exact current
M04-T08 and M04-T09 authorities, authorize aliases and resource instances before materialization,
share one bounded token session across a true operation guard and input, and preserve an
independent M04-T08 session for current resource input. Settlement branches are detached at
acceptance and retained behind finite pending, action-count, and handler-string ceilings.
Started, queued, staged, and refreshed effects remain nonblocking. Terminal operation outcomes
select immutable success or failure work while a private raw lease remains hidden behind one
opaque ticket; mapping alone does not release cumulative capacity or promote queued work.
Eighty-seven focused package tests, twenty-four compiler-negative cases, and nineteen root
proof/mutation tests enforce these semantics, exact manager snapshots, direct lower-handle drift,
terminal ownership, disposal, public/internal API separation, portability, two direct trace
assignments, and eleven task-owned files. M04-T13 still owns ordered handler execution and
`finally` finalization, while M04-T16 owns full cross-manager joint-origin provenance; N-041 stays
`PLANNED` and P-17 stays `PARTIAL`:
`runtime-core-0.1.0-operation-resource-actions.json`
`sha256:5315b174e4161f45eb69a2c5922dde6bddd175e8c467b62c244fd3dde2d669b5`.

M04-T12 defines and proves guarded component-command and outbound host-event actions without
closing P-17 or changing a proof gate. Seven public runtime exports and twenty-five public types
add a receiver-independent synchronous bridge without changing M04-T01's frozen ports. The
prepared Catalog remains the sole command authority; static target, declared command, and exactly
one live runtime instance are selected before input observation. Multiple repeated instances are
retained but never guessed. Exact resolved command input validates before dispatch, while
allowlisted host events select an application contract and validate detached payload before
emission. False guards observe no action payload or callback, and true guards share one bounded
token session with command input or event payload.
...
additional node is rejected before any callback. Fifty-three focused package tests, twenty-one
compiler-negative cases, and nineteen root proof/mutation tests enforce these semantics, six
...
production adapters prove complete declared-command implementation parity. Incoming adapter
events, ordered multi-action turns, full cross-manager provenance, Bundle ingress, activation, and
final finite-limit evidence remain later work; P-17 stays `PARTIAL`:
`runtime-core-0.1.0-command-event-actions.json`
`sha256:a00107d5b33d32b941a33c6ae308373be5a1ccfe1182f7fa4e93993bea2cba19`.
