...
| P-08 | A designer edits props, slots, state, bindings, and actions visually                              | M09-T05–M09-T09, M10-T01                    | NOT_PROVEN     | M09-T05 provides primitive/enum prop controls, M09-T06 adds recursive closed-object controls plus an explicit structured-JSON fallback, and M09-T07 adds Catalog-declared named-slot insertion, cross-slot move, same-slot reorder, and nested-subtree deletion. The T11 compatibility closure makes the dotted grip the actual draggable handle, admits the complete Components panel as the authenticated append surface, owns exactly one global slot/index projection across nested Layers drop surfaces, stabilizes midpoint jitter, uses release coordinates or the last admitted coordinate-less projection, auto-selects an inserted node, and keeps the guarded Delete action immediately visible. That historical T11 behavior is superseded in the current T12 product UX: each compatible component uses a wide draggable card body with a separate `Add` button, only the sticky `Add to` target admits component drops and releases elsewhere are inert, and Layers fences one global owner/epoch projection with midpoint hysteresis and stable 32 px insertion gaps; the guarded `Delete` action remains immediately visible. Named-slot/cardinality/validator authority is unchanged and native real-browser drag E2E remains open. M09-T08 adds deterministic surface-local primitive state list/add/update/delete, bounded conservative usage protection, exact direct Catalog-compatible local-state binding change/detach, and Publisher-atomic preview. Runtime namespaces and advanced dynamic bindings remain read-only. M09-T09 adds Catalog-declared component event projection, exact absent/empty/nonempty handler lifecycle, the closed seven-action union, recursive operation settlements, six public Editor Core edits, and Publisher-atomic preview. Behavior-owner UI remains unclaimed. M09-T10 adds one controlled same-session Design/Run boundary and a local `state.set` execution path, M09-T11 adds transient scenarios and synthetic operation lifecycle, and M09-T12 adds generation-guarded authored Source Open/Save through the injected public Editor Core port. Complete canonical Source content—not identity/version—controls dirty state; same-value/revert cleanliness, successful Open/Save baselines, current-vs-save-snapshot settlement, rerender-safe no-port projection, and the `Local draft unchanged` clean label are protected. Edited no-port and port-backed drafts are guarded across navigation/page exit. Exact own-enumerable settlement capture avoids accessors, freezes valid optional diagnostics, validates CAS relations, fails malformed Open/Save closed, and fences reentrant stale publication. The App owns no concrete storage adapter; diagnostics, publication/activation, and automated real-browser E2E remain unproven. | Desen App browser E2E from an empty project                                                                                       | `desen-app-0.1.0-schema-inspector.json` `sha256:473ab3248ed7b7b4de0e558df47159a74c28c134b46569aa91130745fd69660b`; `desen-app-0.1.0-structured-inspector.json` `sha256:6ea4eb3f51fdfc39eeca676d7ebafb145d66a9efdfa03af9c33a7aa39aa6aaec`; `desen-app-0.1.0-named-slot-authoring.json` `sha256:daae817af45d8ead7052fd84df4edefd7d29cdd9ebe9cc1baea5b22b27dae90f`; `desen-app-0.1.0-state-binding-editor.json` `sha256:b7298375cba4b82258d1c293ecb66c3ae6641408ae9f5753da121ac44fcf601a`; `desen-app-0.1.0-event-action-editor.json` `sha256:0060ef39273ea36666f1701d5d3fa0f1610b95f40d88304ba980dcdc73cb29ab`; `desen-app-0.1.0-fixtures-scenarios-fidelity.json` `sha256:3f08980e687d48ba267f78c7d4dd1ae1eb59db5cc6bb3401d88705ee0416cc9d`; `desen-app-0.1.0-source-persistence.json` `sha256:717d0ddada008edb34909d5defcc4c28e95b36f6dfc0b1abb4d09d9775a6b734` | 2026-08-29    |
...
The current App UX treats each compatible Components card body as one broad native drag source and
keeps click insertion on a separate `Add` button. Only the sticky `Add to` target accepts a
component drop; releases anywhere else are inert. Layers fences one global nested-slot projection
by drag owner and epoch, applies midpoint hysteresis, renders 32 px insertion gaps, and keeps the
guarded `Delete` control visible for the selected node. Native transfer bytes authorize no edit.
...
75/75, 19/19, 22/22, 38/38, and 28/28 locally—182/182 combined. These local receipts make no
