...
6. run all 29 proof verifiers directly in the reviewed order;
7. run all 29 root proof and mutation files as separate fail-fast processes; and
8. run the dependency graph and hostile boundary fixtures.

The current legacy expansion contains 1,065 leaf process invocations but only 103 distinct
workloads. The optimized gate covers all 103 distinct workloads. Repeated prerequisite checks inside proof
...
- the 29 task IDs, verifier files, root test files, or their order;
- any of the 157 legacy prerequisite command segments;
- the exact 66-step normalized execution plan;
