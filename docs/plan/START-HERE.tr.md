...
M10 ve terminal `G10` kapısı tamamen kapandı. Uygulama ilerlemesi 141/176 (%80), M10 12/12, M10A 20/28 (%71) ve kanıt kapıları 11/14 düzeyindedir. M10A-T01–T20 exact-head PR kontrolleri,
merge'leri ve taze `main` koşuları geçti; T20'nin kapanışı [kanıt belgesinde](../proof/M10A-T20.md) kayıtlıdır;
seçilmiş sıradaki görev `M10A-T21` ve `NOT_STARTED` durumundadır.
...
ekler; tamamlanmış görev sayısı 141 ve M10A sayacı 20/28'dir; seçilmiş sıradaki görev `M10A-T21`,
...
`M10A-T15` `DONE`: masters/instances, override/reset/detach ve izole master düzenleme için [hosted exact-head ve taze `main` kapanışı](../proof/M10A-T15.md) geçti. `M10A-T16` `DONE`: named variants ve declared visual-state authoring için [hosted exact-head ve taze `main` kapanışı](../proof/M10A-T16.md) geçti. `M10A-T17` `DONE`: Catalog-backed Design System explorer için [hosted exact-head ve taze `main` kapanışı](../proof/M10A-T17.md) geçti. `M10A-T18` `DONE`: library release/version identity, local usage index, impact comparison, explicit adoption/rejection and restore için [hosted exact-head ve taze `main` kapanışı](../proof/M10A-T17.md#-m10a-t18--library-management-and-impact) geçti.
`M10A-T19` `DONE`: ayrı Connections çalışma alanı ve dayanıklı inert taslaklar için [hosted exact-head ve taze `main` kapanışı](../proof/M10A-T19.md) geçti. `M10A-T20` `DONE`: aynı workspace içinde Source-backed görsel davranış bağlama paneli için [hosted exact-head ve taze `main` kapanışı](../proof/M10A-T20.md) geçti.
`M10A-T21` seçilmiş sıradaki görevdir ve `NOT_STARTED` durumundadır.
...
değişikliği T03 kapsamı dışındadır. Tamamlanan T04 token/asset/recipe snapshot'larını content-addressed
immutable release olarak ayrıştırır; runtime activation, persistent production store ve normal App
entegrasyonu T04 kapsamında değildir.
