...
- Status: Accepted; M10A-T01 through M10A-T06 `DONE`, M10A-T07 `IN_PROGRESS` with local
  evidence and hosted closure pending
- Date: 2026-09-10
...
are `DONE`; T07 is `IN_PROGRESS` with local evidence while the other 21 tasks remain `NOT_STARTED`. T06's hosted
