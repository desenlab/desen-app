...
const DIRECT_PROOF_VERIFIER_PREREQUISITES = Object.freeze({
  ...
  "desen-app-source-persistence": Object.freeze([
    "node scripts/verify-desen-app-shell-navigation.mjs",
    "node scripts/verify-editor-core-persistence.mjs",
...
const QUALITY_GATE_PLAN_SHA256 = "a844173107f2f20175ff242122127fe936a99fa536a35969e619bb087922685c";
...
const EXPECTED_STARTER_PROOF_PACKAGE_SCRIPTS = Object.freeze(
  [
    ...
    [
      "build:m10a-t07",
      "pnpm run prepare:m10a-t07-package && pnpm run build:m10a-t07:authoring && pnpm run build:m10a-t07:host",
    ],
    [
      "build:m10a-t07:authoring",
...
export function validateProofInventory({
  ...
  if (!browserE2ePackage) {
    throw new QualityGateError("The reviewed Browser E2E workspace package is missing.");
  }
  assertExactArray(
    Object.keys(browserE2ePackage.scripts ?? {}).sort(),
  ...
  for (const { name, command } of EXPECTED_BROWSER_E2E_PACKAGE_SCRIPTS) {
    if (browserE2ePackage.scripts[name] !== command) {
      throw new QualityGateError("The Browser E2E workspace package script drifted from review.", {
        script: name,
        expected: command,
  ...
  if (!starterProofPackage) {
    throw new QualityGateError("The reviewed starter browser proof workspace package is missing.");
  }
  assertExactArray(
    Object.keys(starterProofPackage.scripts ?? {}).sort(),
  ...
  for (const { name, command } of EXPECTED_STARTER_PROOF_PACKAGE_SCRIPTS) {
    if (starterProofPackage.scripts[name] !== command) {
      throw new QualityGateError(
        "The starter browser proof workspace package script drifted from review.",
        {
          script: name,
          expected: command,
  ...
  for (const [proofIndex, { id, verifierFile, rootTestFile }] of proofEntries.entries()) {
    if (DIRECT_CI_PROOF_PAIR_IDS.includes(id)) {
      for (const prefix of ["generate:", "verify:", "test:"]) {
        if (Object.hasOwn(scripts, prefix + id)) {
          throw new QualityGateError(
            `${id} must remain a direct CI proof pair outside the root package-script graph.`,
            { script: prefix + id },
          );
        }
      }
      legacyPrerequisiteInventory.push({ id, verify: [], test: [] });
      continue;
    }
