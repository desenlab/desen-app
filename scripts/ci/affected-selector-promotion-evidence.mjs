...
const M08_T08_PROOF_READER_CHECKPOINT = Object.freeze({
  profile: "desen.ci.proof-reader-checkpoints.v1",
  sequence: 36,
  headSha256: "4f351f2f535b1fc335876160256a42b1d53c7c7f43db08e0b86bf50c389d1e82",
  frozenArtifactCount: 33,
  currentReaderCount: 66,
  liveVerification: "PASS",
});
...
export async function verifyAffectedSelectorPromotionEvidence(options = {}) {
  ...
  if (
    ...
    fail(
      ...
      "Current comparison-source receipts do not reproduce the reviewed M08-T08 successor.",
    );
  }
  ...
  if (
    ...
    currentInventory.workloadCount !== 168 ||
    currentInventory.proofUnitCount !== 79
  ) {
    fail(
      "AFFECTED_PROMOTION_SUCCESSOR_AUTHORITY_DRIFT",
      "The current workload graph is not the exact reviewed M08-T08 append-only successor.",
    );
  }
  ...
  if (currentProofPairClasses.ordinary !== 68 || currentProofPairClasses.barrier !== 11) {
    fail(
      "AFFECTED_PROMOTION_SUCCESSOR_AUTHORITY_DRIFT",
      "The current M08-T08 proof-pair authority is not exactly 68 ordinary and 11 barrier pairs.",
    );
  }
...
export function validateAffectedSelectorPromotionLiveCheckpoint(liveReceipt) {
  const projection =
    liveReceipt !== null && typeof liveReceipt === "object" && !Array.isArray(liveReceipt)
      ...
      : null;
  if (!isDeepStrictEqual(projection, M08_T08_PROOF_READER_CHECKPOINT)) {
    fail(
      "AFFECTED_PROMOTION_CUTOVER_DRIFT",
      "Promotion evidence does not match the live proof-reader checkpoint authority.",
      { expected: M08_T08_PROOF_READER_CHECKPOINT, actual: projection },
    );
  }
  return liveReceipt;
}
