...
const M08_SUCCESSOR_ADDED_TRACKED_PATHS = Object.freeze([
...
const CURRENT_SUCCESSOR_OWNERSHIP_REVIEW = Object.freeze({
  ...
  categoryCounts: Object.freeze({
    ...
    DEPENDENCY_POLICY: 31,
    FROZEN_INPUT: 119,
    PACKAGE_OR_APPLICATION: 408,
    SHARED_PROOF_INFRASTRUCTURE: 187,
    PROJECT_DOCUMENTATION: 111,
    REPOSITORY_POLICY: 11,
  }),
...
const M08_T04_PROOF_READER_CHECKPOINT = Object.freeze({
  profile: "desen.ci.proof-reader-checkpoints.v1",
  sequence: 32,
  headSha256: "9be019b902ee17a57c9e2f13270fa67fe26265d06e360719bd1542643be6a424",
  frozenArtifactCount: 29,
  currentReaderCount: 58,
  liveVerification: "PASS",
});
...
function createBoundaryOwnershipDelta(rawBoundary) {
  ...
  for (const trackedPath of M08_SUCCESSOR_ADDED_TRACKED_PATHS) {
    if (!successorPaths.includes(trackedPath)) {
      fail(
        "AFFECTED_PROMOTION_OWNERSHIP_EQUIVALENCE_DRIFT",
        "The authenticated boundary omitted one exact M08 successor path.",
        { path: trackedPath },
      );
    }
  }
  const promotedPaths = successorPaths.filter(
    (trackedPath) => !M08_SUCCESSOR_ADDED_TRACKED_PATHS.includes(trackedPath),
  );
  ...
  if (!isDeepStrictEqual(promotedReview, PROMOTED_OWNERSHIP_REVIEW)) {
    fail(
      "AFFECTED_PROMOTION_OWNERSHIP_EQUIVALENCE_DRIFT",
      "Removing the exact M08 append does not reproduce the promoted I07-04 ownership authority.",
    );
  }
...
export async function verifyAffectedSelectorPromotionEvidence(options = {}) {
  ...
  if (
    ...
    fail(
      ...
      "Current comparison-source receipts do not reproduce the reviewed M08-T04 successor.",
    );
  }
  ...
  if (
    ...
    currentInventory.workloadCount !== 159 ||
    currentInventory.proofUnitCount !== 75
  ) {
    fail(
      "AFFECTED_PROMOTION_SUCCESSOR_AUTHORITY_DRIFT",
      "The current workload graph is not the exact reviewed M08-T04 append-only successor.",
    );
  }
  ...
  if (currentProofPairClasses.ordinary !== 64 || currentProofPairClasses.barrier !== 11) {
    fail(
      "AFFECTED_PROMOTION_SUCCESSOR_AUTHORITY_DRIFT",
      "The current M08-T04 proof-pair authority is not exactly 64 ordinary and 11 barrier pairs.",
    );
  }
...
export function validateAffectedSelectorPromotionLiveCheckpoint(liveReceipt) {
  const projection =
    liveReceipt !== null && typeof liveReceipt === "object" && !Array.isArray(liveReceipt)
      ...
      : null;
  if (!isDeepStrictEqual(projection, M08_T04_PROOF_READER_CHECKPOINT)) {
    fail(
      "AFFECTED_PROMOTION_CUTOVER_DRIFT",
      "Promotion evidence does not match the live proof-reader checkpoint authority.",
      { expected: M08_T04_PROOF_READER_CHECKPOINT, actual: projection },
    );
  }
  return liveReceipt;
}
