...
const PARENT_PLAN_SHA256 = "4eb596fa3ec8ceec77466321e3ab408ecfaa184ef5b7c8dadc830c0c5f816546";
...
/**
 * Builds the fixed 53/32/36 proof-pair partition from the authenticated 256-node inventory.
 * Its logical coverage remains 256; three prefixes and the join's fresh build execute 281
...
export function createShardedQualityGatePlan(rawOptions = undefined) {
  ...
  if (inventory.workloadCount !== 256 || inventory.proofUnitCount !== 121) {
  ...
  if (
    new Set(shardProjections.flatMap(({ proofPairIds }) => proofPairIds)).size !== 121 ||
  ...
  if (covered.size !== 256 || logicalNodeIds.some((id) => !covered.has(id))) {
