...
const EXPECTED_PLAN_SHA256_BY_AUTHORITY = Object.freeze({
  REQUIRED: "53985101ccb9727df7e1ff8f8c3c90a67b980618b989d8c5f9d5c0a7ca1932a2",
  SHADOW: "8301b21b49c3f53f15e2b5752da0c7bbb1da69cce8fc7de3929ac7c6f574036c",
});
...
function deriveExecutionRegions(inventory) {
  ...
  if (
    ...
    fail(
      "REQUIRED_EXHAUSTIVE_REGION_INVALID",
      "The derived execution regions do not own all 202 workloads exactly once.",
    );
  }
...
/**
 * Builds the single code-owned exhaustive plan from the authenticated 202-node dependency graph.
 *
...
function validatePlanForExecution(candidate, expectedAuthority) {
  ...
  if (candidate.concurrency !== PROOF_PAIR_CONCURRENCY || candidate.stepCount !== 202) {
    fail(
      "REQUIRED_EXHAUSTIVE_PLAN_DRIFT",
...
async function runProofPairRegion(
  ...
  flushOrdinarySegment();
  if (barrierCount !== 11 || plan.proofPairs.length - barrierCount !== 85) {
    fail(
      "REQUIRED_EXHAUSTIVE_CLASS_DRIFT",
      "The shared-state authority must classify exactly 85 ordinary pairs and 11 barrier pairs.",
 ...
 * receipt remains in the stable 202-node inventory order even though proof pairs may overlap.
 */
export async function runRequiredExhaustivePlan(
  ...
  if (receipt.status !== "PASS" || completedIds.size !== 202) {
    throw attachExecutionReceipt(
      new RequiredExhaustiveQualityGateError(
        "REQUIRED_EXHAUSTIVE_RECEIPT_INCOMPLETE",
        "The exhaustive gate did not observe all 202 workloads close successfully.",
...
function printableReceipt(boundaryReceipt, error) {
  ...
  return {
    ...
    stepCount: execution?.stepCount ?? 202,
    proofPairCount: execution?.proofPairCount ?? 96,
