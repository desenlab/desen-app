...
test("the dependency-derived plan owns the exact 192-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 192);
  assert.equal(plan.proofPairCount, 91);
  assert.equal(plan.prefix.length, 8);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "cffce400b6a3793d3e42051508425bf81a0c66923b6e579d40ac82b836e8daec");
  assert.equal(ownedIds.length, 192);
  assert.equal(new Set(ownedIds).size, 192);
...
test("authority defaults to REQUIRED, accepts only explicit SHADOW, and fixes EXHAUSTIVE scope", () => {
  ...
  assert.equal(
    ...
    "55a04119bc9b6a3041d24ede83d36613ac7dac98cd871cded240387952ce750e",
  );
  assert.throws(
...
test("all 192 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(new Set(calls).size, 192);
  ...
  assert.equal(receipt.observedClosedCount, 192);
  assert.deepEqual(
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 80);
...
test("untracked drift fails the gate even after all 192 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 192);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 192);
