...
test("the dependency-derived plan owns the exact 210-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 210);
  assert.equal(plan.proofPairCount, 100);
  assert.equal(plan.prefix.length, 8);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "5e7c1f5c6add7c75bf950c37ed031f8279262f38e632f77a29b26510be6b79bc");
  assert.equal(ownedIds.length, 210);
  assert.equal(new Set(ownedIds).size, 210);
...
test("authority defaults to REQUIRED, accepts only explicit SHADOW, and fixes EXHAUSTIVE scope", () => {
  ...
  assert.equal(
    ...
    "0fcf1665d264601c5e0f4da95c14863525cdc180dc8d00af27cb5ec5c8cfa147",
  );
  assert.throws(
...
test("all 210 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(calls.length, 210);
  assert.equal(new Set(calls).size, 210);
  ...
  assert.equal(receipt.observedClosedCount, 210);
  assert.deepEqual(
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 89);
...
test("untracked drift fails the gate even after all 210 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 210);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 210);
