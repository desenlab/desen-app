...
test("the dependency-derived plan owns the exact 194-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 194);
  assert.equal(plan.proofPairCount, 92);
  assert.equal(plan.prefix.length, 8);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "e0933cb5f272fbc2aba058ac5a6c256a23f14bc4cfe8018c8b919c3851f92cac");
  assert.equal(ownedIds.length, 194);
  assert.equal(new Set(ownedIds).size, 194);
...
test("authority defaults to REQUIRED, accepts only explicit SHADOW, and fixes EXHAUSTIVE scope", () => {
  ...
  assert.equal(
    ...
    "01aadef839eec54e43b252d4bdfea183ac0256806399d3f71c898451fa7a33ff",
  );
  assert.throws(
...
test("all 194 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(calls.length, 194);
  assert.equal(new Set(calls).size, 194);
  ...
  assert.equal(receipt.observedClosedCount, 194);
  assert.deepEqual(
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 81);
...
test("untracked drift fails the gate even after all 194 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 194);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 194);
