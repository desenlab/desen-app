...
test("the dependency-derived plan owns the exact 148-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 148);
  assert.equal(plan.proofPairCount, 70);
  assert.equal(plan.prefix.length, 6);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "18f7cb0ab88d0847afb6f030c80ef6367c3a599466a22b1cf8826055d5d44502");
  assert.equal(ownedIds.length, 148);
  assert.equal(new Set(ownedIds).size, 148);
...
test("all 148 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(new Set(calls).size, 148);
  assert.equal(receipt.status, "PASS");
  assert.equal(receipt.observedClosedCount, 148);
  assert.deepEqual(
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 59);
...
test("untracked drift fails the gate even after all 148 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 148);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 148);
