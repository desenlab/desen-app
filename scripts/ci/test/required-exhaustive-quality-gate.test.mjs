...
test("the dependency-derived plan owns the exact 252-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 252);
  assert.equal(plan.proofPairCount, 119);
  assert.equal(plan.prefix.length, 12);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "131b340fad0b05b4872d5ca03ec6b5541a510d9b330426048de78fdea5060418");
  assert.equal(ownedIds.length, 252);
  assert.equal(new Set(ownedIds).size, 252);
...
test("authority defaults to REQUIRED, accepts only explicit SHADOW, and fixes EXHAUSTIVE scope", () => {
  ...
  assert.equal(
    ...
    "2554354774da6d3ea792aba4292721c6f4b580caa6808b3b3afc11c23558ec23",
  );
  assert.throws(
...
test("all 252 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(calls.length, 252);
  assert.equal(new Set(calls).size, 252);
  ...
  assert.equal(
    calls.filter((id) => id === "design-system-release-public-package-contract").length,
    1,
  );
  assert.equal(
  ...
  assert.equal(receipt.observedClosedCount, 252);
  assert.deepEqual(
...
test("only the exact digest and published-host pairs move within their ordinary segments without changing authority", async () => {
  ...
  const segmentBarrierIndex = plan.proofPairs.findIndex(
    ({ id }) => id === "reference-host-web-source-audit",
  );
  const successorBarrierIndex = plan.proofPairs.findIndex(({ id }) => id === "m10a-t03");
  const segmentBarrier = plan.proofPairs[segmentBarrierIndex];
  const finalSegment = plan.proofPairs.slice(segmentBarrierIndex + 1, successorBarrierIndex);
  assert.equal(classifyProofPairState(segmentBarrier.id).barrier, true);
  assert.equal(classifyProofPairState(plan.proofPairs[successorBarrierIndex].id).barrier, true);
  assert.equal(finalSegment.at(-8).id, "desen-app-published-host-update");
  assert.equal(finalSegment.at(-7).id, "desen-app-invalid-publication");
  assert.equal(finalSegment.at(-6).id, "desen-app-last-known-good-recovery");
  assert.equal(finalSegment.at(-5).id, "desen-app-repeatable-demo");
  assert.equal(finalSegment.at(-4).id, "runtime-core-baseline");
  assert.equal(finalSegment.at(-3).id, "m10-gate");
  assert.equal(finalSegment.at(-2).id, "m10a-t01");
  assert.equal(finalSegment.at(-1).id, "m10a-t02");
  const finalSegmentStart = ids.indexOf(segmentBarrier.rootTest.id) + 1;
  ...
  for (const { verifier, rootTest } of plan.proofPairs.slice(0, segmentBarrierIndex + 1)) {
...
test("a held early published-host root overlaps its own segment and blocks the successor barrier and serial suffix", async () => {
  const plan = createShadowPlan();
  const segmentBarrierIndex = plan.proofPairs.findIndex(
    ({ id }) => id === "reference-host-web-source-audit",
  );
  const successorBarrierIndex = plan.proofPairs.findIndex(({ id }) => id === "m10a-t03");
  const segment = plan.proofPairs.slice(segmentBarrierIndex + 1, successorBarrierIndex);
  const publishedPair = segment.at(-8);
  ...
  try {
    ...
    assert.equal(segment.length, 63);
    assert.equal(started.includes(publishedPair.rootTest.id), true);
    assert.equal(completed.has(publishedPair.rootTest.id), false);
    assert.equal(completed.has(plan.proofPairs[segmentBarrierIndex].rootTest.id), true);
    assert.equal(
      started.indexOf(publishedPair.rootTest.id) < started.indexOf(segment.at(-2).rootTest.id),
      true,
    );
    for (const { id } of [
      plan.proofPairs[successorBarrierIndex].verifier,
      plan.proofPairs[successorBarrierIndex].rootTest,
      ...plan.suffix,
    ]) {
      assert.equal(started.includes(id), false);
    }
  } finally {
    releasePublished();
  }
...
test("cancelling the early published-host root drains its sibling without launching the successor barrier or final tail", async () => {
  const plan = createShadowPlan();
  const segmentBarrierIndex = plan.proofPairs.findIndex(
    ({ id }) => id === "reference-host-web-source-audit",
  );
  ...
  await rejected;
  const finalSegmentStart = started.indexOf(plan.proofPairs[segmentBarrierIndex].rootTest.id) + 1;
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(barrierPairs.length, 15);
  assert.equal(plan.proofPairs.length - barrierPairs.length, 104);
  assert.equal(exclusiveBarrierSteps.size, 30);
});
...
test("untracked drift fails the gate even after all 252 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 252);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 252);
