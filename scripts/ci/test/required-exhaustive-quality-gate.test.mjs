...
test("the dependency-derived plan owns the exact 224-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 224);
  assert.equal(plan.proofPairCount, 107);
  assert.equal(plan.prefix.length, 8);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "a69bc698eb3677ca6ec62a1c553df74a390c4c43ab14aabc20f75370c938395c");
  assert.equal(ownedIds.length, 224);
  assert.equal(new Set(ownedIds).size, 224);
  ...
  assert.equal(plan.proofPairs.at(-4).id, "historical-archive-redaction");
  assert.equal(plan.proofPairs.at(-4).verifier.id, "verify-historical-archive-redaction");
  assert.equal(plan.proofPairs.at(-4).rootTest.id, "test-historical-archive-redaction");
  assert.equal(plan.proofPairs.at(-3).id, "desen-app-published-host-update");
  assert.equal(plan.proofPairs.at(-3).verifier.id, "verify-desen-app-published-host-update");
  assert.equal(plan.proofPairs.at(-3).rootTest.id, "test-desen-app-published-host-update");
  assert.equal(plan.proofPairs.at(-2).id, "desen-app-invalid-publication");
  assert.equal(plan.proofPairs.at(-2).verifier.id, "verify-desen-app-invalid-publication");
  assert.equal(plan.proofPairs.at(-2).rootTest.id, "test-desen-app-invalid-publication");
  assert.equal(plan.proofPairs.at(-1).id, "desen-app-last-known-good-recovery");
  assert.equal(plan.proofPairs.at(-1).verifier.id, "verify-desen-app-last-known-good-recovery");
  assert.equal(plan.proofPairs.at(-1).rootTest.id, "test-desen-app-last-known-good-recovery");
...
test("authority defaults to REQUIRED, accepts only explicit SHADOW, and fixes EXHAUSTIVE scope", () => {
  ...
  assert.equal(
    ...
    "0c9cdd39315c665be8ec00746f90fae2267d58647f45a175325f71f6b0d66d19",
  );
  assert.throws(
...
test("all 224 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(calls.length, 224);
  assert.equal(new Set(calls).size, 224);
  ...
  assert.equal(receipt.observedClosedCount, 224);
  assert.deepEqual(
...
test("only the exact digest and published-host pairs move within their ordinary segments without changing authority", async () => {
  ...
  assert.equal(finalSegment.at(-3).id, "desen-app-published-host-update");
  assert.equal(finalSegment.at(-2).id, "desen-app-invalid-publication");
  assert.equal(finalSegment.at(-1).id, "desen-app-last-known-good-recovery");
...
test("a held early published-host root overlaps its own segment and blocks the serial suffix", async () => {
  ...
  const publishedPair = segment.at(-3);
  ...
  try {
    ...
    assert.equal(segment.length, 58);
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 96);
...
test("untracked drift fails the gate even after all 224 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 224);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 224);
