...
test("the dependency-derived plan owns the exact 184-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 184);
  assert.equal(plan.proofPairCount, 87);
  assert.equal(plan.prefix.length, 8);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "89c68ec52affb10717459bb2dea41cd1608af2b43212db7dc54bdaaab95f1433");
  assert.equal(ownedIds.length, 184);
  assert.equal(new Set(ownedIds).size, 184);
...
test("all 184 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(calls.length, 184);
  assert.equal(new Set(calls).size, 184);
  ...
  assert.equal(receipt.observedClosedCount, 184);
  assert.deepEqual(
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 76);
...
test("untracked drift fails the gate even after all 184 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 184);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 184);
