...
test("the dependency-derived plan owns the exact 178-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 178);
  assert.equal(plan.proofPairCount, 84);
  assert.equal(plan.prefix.length, 8);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "ef2b2dcec3721dbeab30a50a04b02d08b853e07a3f01b5acd793674910cd473f");
  assert.equal(ownedIds.length, 178);
  assert.equal(new Set(ownedIds).size, 178);
...
test("all 178 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(calls.length, 178);
  assert.equal(new Set(calls).size, 178);
  ...
  assert.equal(receipt.observedClosedCount, 178);
  assert.deepEqual(
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 73);
...
test("untracked drift fails the gate even after all 178 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 178);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 178);
