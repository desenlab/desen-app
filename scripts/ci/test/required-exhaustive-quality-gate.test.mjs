...
test("the dependency-derived plan owns the exact 212-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 212);
  assert.equal(plan.proofPairCount, 101);
  assert.equal(plan.prefix.length, 8);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "8853118a28eb662e2292932a4389089990d13855856757b320186c88e32272c1");
  assert.equal(ownedIds.length, 212);
  assert.equal(new Set(ownedIds).size, 212);
...
test("authority defaults to REQUIRED, accepts only explicit SHADOW, and fixes EXHAUSTIVE scope", () => {
  ...
  assert.equal(
    ...
    "8b99b7f3a426e0ec31178f3bce672d9bde3ab91dae8636f168f3bfe5fa9e1ae6",
  );
  assert.throws(
...
test("all 212 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(receipt.observedClosedCount, 212);
  assert.deepEqual(
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 90);
...
test("untracked drift fails the gate even after all 212 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 212);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 212);
