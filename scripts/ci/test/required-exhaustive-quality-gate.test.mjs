...
test("the dependency-derived plan owns the exact 186-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 186);
  assert.equal(plan.proofPairCount, 88);
  assert.equal(plan.prefix.length, 8);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "00c0fbb14b15bf898872ba09b61c9a3d8995f60f42cf1bf894a0acb096dc2490");
  assert.equal(ownedIds.length, 186);
  assert.equal(new Set(ownedIds).size, 186);
...
test("all 186 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(new Set(calls).size, 186);
  ...
  assert.equal(receipt.observedClosedCount, 186);
  assert.deepEqual(
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 77);
...
test("untracked drift fails the gate even after all 186 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 186);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 186);
