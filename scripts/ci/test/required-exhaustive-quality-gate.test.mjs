...
test("the dependency-derived plan owns the exact 159-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 159);
  assert.equal(plan.proofPairCount, 75);
  assert.equal(plan.prefix.length, 7);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "30a193cbc27316792bd577dcecdc87c10e680e2e033698ceb90787c2cbcf1b51");
  assert.equal(ownedIds.length, 159);
  assert.equal(new Set(ownedIds).size, 159);
...
test("all 159 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(calls.length, 159);
  assert.equal(new Set(calls).size, 159);
  ...
  assert.equal(receipt.observedClosedCount, 159);
  assert.deepEqual(
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 64);
...
test("untracked drift fails the gate even after all 159 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 159);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 159);
