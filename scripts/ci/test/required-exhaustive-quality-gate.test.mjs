...
test("the dependency-derived plan owns the exact 165-node exhaustive inventory", () => {
  ...
  assert.equal(plan.stepCount, 165);
  assert.equal(plan.proofPairCount, 78);
  assert.equal(plan.prefix.length, 7);
  assert.equal(plan.suffix.length, 2);
  assert.equal(plan.planSha256, "5484324b6d22a5e58bce2431f35382aeeb4e97095c96524e5bdb6211f8650a9e");
  assert.equal(ownedIds.length, 165);
  assert.equal(new Set(ownedIds).size, 165);
...
test("all 165 successful closes produce stable inventory-ordered receipts", async () => {
  const plan = createShadowPlan();
  const calls = [];
  ...
  assert.equal(calls.length, 165);
  assert.equal(new Set(calls).size, 165);
  ...
  assert.equal(receipt.observedClosedCount, 165);
  assert.deepEqual(
...
test("dynamic workers keep two safe ordinary pairs active and drain for all barrier pairs", async () => {
  ...
  assert.equal(plan.proofPairs.length - barrierPairs.length, 67);
...
test("untracked drift fails the gate even after all 165 steps close successfully", async () => {
  const plan = createShadowPlan();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.requiredExhaustiveReceipt.status, "FAIL");
      assert.equal(error.requiredExhaustiveReceipt.observedClosedCount, 165);
      return true;
    },
  );
});
...
test("the full gate authenticates repository inputs and hosted revision without executing commands", async () => {
  ...
  assert.equal(receipt.execution.observedClosedCount, 165);
