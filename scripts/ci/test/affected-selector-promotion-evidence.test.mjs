...
test("authenticates the exact 20/20 hosted campaign and live local-preflight successor", async () => {
  ...
  assert.deepEqual(
    checkpoint.checkpoints
      .at(-1)
      .artifacts.find(
        ({ path: artifactPath }) => artifactPath === "docs/proof/artifacts/m10a-t09.json",
      ),
    {
      task: "M10A-T09",
  ...
  assert.deepEqual(
    checkpoint.checkpoints
      .at(-1)
      .artifacts.find(
        ({ path: artifactPath }) => artifactPath === "docs/proof/artifacts/m10a-t12.json",
      ),
    {
      task: "M10A-T12",
  ...
  assert.deepEqual(receipt.promotedAuthorities, {
    selectorSha256: "4c8c0a84eceaa2db790e93042af1e052a0f9853bccc866d89e3072ad324fe8eb",
    ownershipSha256: "229cf86ef50580fb35b87ed07e540bec3a9727a55cee768c29d6285ec6ec8c31",
    impactGraphSha256: "97297c21c1ffbc0270beaf15f732528928f216a869e39c6eee824b9b2874f1b0",
    ...
    runnerAuthoritySha256: "fe4e311b1345fe2fc3535b1deeca0dd1eb9507722d35a27e561f4de7d9048805",
  });
});
...
test("rejects rollback of every live execution timeout boundary", async () => {
  for (const [relativePath, currentFragment, staleFragment, occurrences = 1] of [
    ...
    [".github/workflows/ci.yml", "timeout-minutes: 33", "timeout-minutes: 25", 4],
    [
      "scripts/ci/run-required-exhaustive-quality-gate.mjs",
      "const DEFAULT_GATE_TIMEOUT_MS = 27 * 60 * 1_000;",
...
test("rejects an unreviewed widening of the live exhaustive soft deadline", async () => {
  ...
  try {
    ...
    const currentFragment = "const DEFAULT_GATE_TIMEOUT_MS = 27 * 60 * 1_000;";
    const source = await readFile(target, "utf8");
