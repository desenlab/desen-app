...
test("the neutral inventory preserves the exact 256-workload T13 successor projection", () => {
  ...
  assert.equal(inventory.workloadCount, 256);
  assert.equal(inventory.proofUnitCount, 121);
  ...
  assert.equal(
    projectionSha256,
    "a75f65e05020b75b2d4253b291270af3bb9f29db423287ed4c9a3bcef81e2105",
  );
  assert.deepEqual(
  ...
  assert.equal(
    inventory.nodes.slice(12, 133).every(({ id }) => id.startsWith("verify-")),
    true,
  );
  assert.equal(
    inventory.nodes.slice(133, 254).every(({ id }) => id.startsWith("test-")),
    true,
  );
  assert.deepEqual(
  ...
  const t12SuccessorPredecessor = projection.filter(
    ({ id }) => !id.endsWith("m10a-t12") && !id.endsWith("m10a-t13"),
  );
  assert.equal(t12SuccessorPredecessor.length, 252);
  assert.equal(
    createHash("sha256").update(JSON.stringify(t12SuccessorPredecessor)).digest("hex"),
    "ce98905d47e8a0bc8c8bfcc3a36bb6f218922082cd05412f9b1c89c9d3eb56c3",
  );
  const t09SuccessorPredecessor = t12SuccessorPredecessor.filter(
    ({ id }) => !id.endsWith("m10a-t09"),
  );
  assert.equal(t09SuccessorPredecessor.length, 250);
  assert.equal(
    createHash("sha256").update(JSON.stringify(t09SuccessorPredecessor)).digest("hex"),
    "49a997c292d32641c4f3530c19d7863f6ef80d16a337b8e5fa22566b6dfbd7af",
  );
  const t08SuccessorPredecessor = t09SuccessorPredecessor.filter(
    ({ id }) => !id.endsWith("m10a-t08"),
  );
  ...
  const t06SuccessorPredecessor = t07SuccessorPredecessor.filter(
    ({ id }) => !id.endsWith("m10a-t06"),
  );
...
test("repository manifests and discovered proof files retain the reviewed parity receipt", async () => {
  ...
  assert.deepEqual(receipt, {
    ...
    legacyPrerequisiteSha256: "e14ac458cd6e0aff4ea894e9faf65f9981224778f6394b02f2bba50573d2cab4",
    legacyLeafInvocationCount: 4660,
    legacyLeafInvocationSha256: "4dc6ef205b56d615efe037f862989ba8f67302f96cad2e4ec78aa862d3616db6",
    distinctLeafWorkloadCount: 381,
    distinctLeafWorkloadSha256: "b78f2b3574b6d56dcc6dbff5c7a54c285d916bbf562161782deec17fe497ae1f",
...
test("dependencies, execution classes, and shared-state ownership are explicit", () => {
  ...
  assert.equal(boundaries.dependencies.length, 121);
  ...
  for (const unit of inventory.proofUnits) {
    ...
    assert.deepEqual(verifier.dependencies, [
      unit.id === "m10a-t07" || unit.id === "m10a-t08" || unit.id === "m10a-t09"
        ? "starter-catalog-web-public-package-contract"
        : unit.id === "m10a-t04"
          ? "design-system-release-public-package-contract"
          : unit.id === "m10a-t03"
...
test("M10A-T05 and T06 are historical read-only proof pairs while T07 retains current proof isolation", () => {
  ...
  assert.deepEqual(nodeById.get("test-m10a-t06")?.sharedState, {
    trackedWorkspace: "READ_ONLY_GUARDED",
    buildOutputs: "NONE",
    temporaryPaths: "NONE",
    ports: "NONE",
  });
  assert.deepEqual(nodeById.get("verify-m10a-t07")?.sharedState, {
    trackedWorkspace: "READ_ONLY_GUARDED",
    buildOutputs: "NONE",
