...
test("runs the persistence closure behind all three public-package contracts", async () => {
  const selection = createRequiredAffectedSelection(
    await boundary("scripts/verify-editor-core-persistence.mjs"),
  );
  const receipt = await runRequiredAffectedQualityGate(selection, { runStep: runner() });

  assert.equal(selection.proofUnitCount, 80);
  assert.equal(selection.workloadCount, 171);
  assert.deepEqual(selection.nodeIds.slice(0, 9), [
  ...
  assert.equal(receipt.status, "PASS");
  assert.equal(receipt.observedClosedCount, 171);
});
...
test("required exhaustive invariants remain exact after required execution is imported", () => {
  ...
  assert.equal(required.stepCount, 235);
  assert.equal(required.proofPairCount, 112);
});
