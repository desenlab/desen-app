...
test("runs the persistence closure behind both public-package contracts", async () => {
  ...
  assert.equal(selection.proofUnitCount, 59);
  assert.equal(selection.workloadCount, 128);
  ...
  assert.equal(receipt.status, "PASS");
  assert.equal(receipt.observedClosedCount, 128);
});
...
test("required exhaustive invariants remain exact after required execution is imported", () => {
  ...
  assert.equal(required.stepCount, 194);
  assert.equal(required.proofPairCount, 92);
});
