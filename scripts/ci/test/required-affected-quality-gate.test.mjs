...
test("runs the persistence closure behind both public-package contracts", async () => {
  ...
  assert.equal(selection.proofUnitCount, 54);
  assert.equal(selection.workloadCount, 118);
  ...
  assert.equal(receipt.status, "PASS");
  assert.equal(receipt.observedClosedCount, 118);
});
...
test("required exhaustive invariants remain exact after required execution is imported", () => {
  ...
  assert.equal(required.stepCount, 184);
  assert.equal(required.proofPairCount, 87);
});
