...
test("CI-04 three fixed shards preserve all 238 workloads and 113 complete proof pairs", () => {
  ...
  assert.equal(pairIds.length, 113);
  assert.equal(new Set(pairIds).size, 113);
  assert.deepEqual(
    pairIds,
  ...
  for (const shard of shards) {
    ...
    assert.equal(shard.prefix.length, 10);
    assert.deepEqual(shard.prefix, full.prefix);
    assert.deepEqual(shard.suffix, []);
    assert.equal(shard.stepCount, 10 + shard.proofPairCount * 2);
...
test("CI-04 static shard membership agrees with executable membership and stays immutable", () => {
  ...
  assert.equal(partition.logicalWorkloadCount, 238);
  assert.equal(partition.physicalWorkloadCount, 259);
  assert.equal(partition.repeatedPrefixWorkloadCount, 20);
