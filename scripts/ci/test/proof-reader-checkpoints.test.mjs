...
async function assertHistoricalReaderMatchesCurrentWorkspace(reader, index) {
  ...
  if (M10A_T18_RESEALED_READER_INDEXES.includes(index)) {
    expectedCurrent = baselineManifest.checkpoints[149].readers[index];
  }
  if (
...
test("the reviewed chain authenticates its immutable genesis and current readers", async () => {
  ...
  assert.equal(manifest.checkpoints.length, 150);
  ...
  assert.equal(manifest.headSha256, PROOF_READER_CHECKPOINT_REVIEWED_CHAIN_SHA256[149]);
  assert.equal(PROOF_READER_CHECKPOINT_REVIEWED_CHAIN_SHA256.length, 150);
  assert.equal(
    PROOF_READER_CHECKPOINT_REVIEWED_CHAIN_SHA256[7],
