...
test("the reviewed chain authenticates its immutable genesis and current readers", async () => {
  ...
  assert.equal(manifest.checkpoints.length, 32);
  ...
  assert.equal(manifest.headSha256, PROOF_READER_CHECKPOINT_REVIEWED_CHAIN_SHA256[31]);
  assert.equal(PROOF_READER_CHECKPOINT_REVIEWED_CHAIN_SHA256.length, 32);
  assert.equal(
    PROOF_READER_CHECKPOINT_REVIEWED_CHAIN_SHA256[7],
  ...
  assert.equal(
    ...
    "ef02e9a20725159352950131c8e9a575a7b4185a55968fd1ec22f42a85571aee",
  );
...
test("sequence thirty-two reseals every current editor reader and appends exact M08-T04 authority", async () => {
  ...
  assert.deepEqual(current.readers.slice(50), [
    ...
    },
    {
      ...
      bytes: 76036,
      sha256: "eff6221217a81e938ed1fa7436b7f28db0b7a8a2f1f4c4d4cc8879f40702161c",
    },
    {
      task: "M08-T04",
      role: "root-test",
      ...
      bytes: 21358,
      sha256: "f5c56204486a028cac6866b06261f5144fae5459a52a1e4ac344e22acac9c9fb",
    },
  ]);

  const authorities = [...current.artifacts, ...current.readers];
  const liveReceipts = await Promise.all(
    authorities.map(async ({ task, role, path: authorityPath }) => {
...
test("task, artifact, reader path, role, and order authority are owned by code", () => {
  ...
  assert.equal(PROOF_READER_CHECKPOINT_TASK_AUTHORITY.length, 29);
...
test("one changed reader is a valid review candidate while fifty-seven peers remain unchanged", () => {
  const manifest = cloneBaseline();
