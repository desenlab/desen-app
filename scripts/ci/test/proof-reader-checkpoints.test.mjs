...
test("the reviewed chain authenticates its immutable genesis and current readers", async () => {
  ...
  assert.equal(manifest.checkpoints.length, 34);
  ...
  assert.equal(manifest.headSha256, PROOF_READER_CHECKPOINT_REVIEWED_CHAIN_SHA256[33]);
  assert.equal(PROOF_READER_CHECKPOINT_REVIEWED_CHAIN_SHA256.length, 34);
  assert.equal(
    PROOF_READER_CHECKPOINT_REVIEWED_CHAIN_SHA256[7],
...
test("sequence thirty-four reseals affected editor readers and appends exact M08-T06 authority", async () => {
...
test("task, artifact, reader path, role, and order authority are owned by code", () => {
  ...
  assert.equal(PROOF_READER_CHECKPOINT_TASK_AUTHORITY.length, 31);
...
test("one changed reader is a valid review candidate while sixty-one peers remain unchanged", () => {
  const manifest = cloneBaseline();
