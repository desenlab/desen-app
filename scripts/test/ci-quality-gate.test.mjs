...
const EXPECTED_COMPLETED_IMPLEMENTATION_TASKS = 139;
...
test("task board retains its canonical inventory without narrative appendices", async () => {
  ...
  assert.equal(completedM10ATasks, 18);
  assert.equal(completionPercent, 79);
  assert.equal(m10aCompletionPercent, 64);
  ...
  assert.ok(
    normalizedReadme.includes(
      "**Next task:** `M10A-T19` (`NOT_STARTED`; Connections workspace and safe drafts)",
    ),
  );
  assert.ok(normalizedProjectStatus.includes("M10A-T01 through M10A-T18 are DONE"));
  assert.ok(
    normalizedFullProjectStatus.includes(
      "M10A-T19 — Connections workspace and safe drafts — is the selected next task",
    ),
  );
  assert.ok(
    normalizedFullProjectStatus.includes("M10A-T20 is also dependency-ready but unselected"),
  );
  ...
  assert.ok(
    normalizedStartHere.includes(
      "T19 seçilmiş sıradaki görevdir; T20 de bağımlılık açısından hazırdır; ikisi de `NOT_STARTED`.",
    ),
  );
  assert.equal(
