...
const EXPECTED_COMPLETED_IMPLEMENTATION_TASKS = 141;
...
async function currentInventory() {
  ...
  const configurationPattern =
    /^(?:(?:t0[6789]-)?vite\.config|vitest\.config|vitest\.workspace)\.[^/]+$/u;
...
test("task board retains its canonical inventory without narrative appendices", async () => {
  ...
  assert.equal(completedM10ATasks, 20);
  assert.equal(completionPercent, 80);
  assert.equal(m10aCompletionPercent, 71);
  ...
  const normalizedFullProjectStatus = normalizeWhitespace(projectStatus);
  ...
  assert.equal(statuses.get("M10A-T17"), "DONE");
  assert.equal(statuses.get("M10A-T18"), "DONE");
  assert.equal(statuses.get("M10A-T19"), "DONE");
  assert.equal(statuses.get("M10A-T20"), "DONE");
  ...
  assert.ok(normalizedProjectStatus.includes("M10A-T01 through M10A-T20 are DONE"));
  assert.ok(
    normalizedFullProjectStatus.includes(
      "M10A-T21 — resource/operation connections and explicit host readiness — is the selected next task",
    ),
  );
  ...
  assert.ok(
    normalizedStartHere.includes(
      "`M10A-T09` `DONE`: Card, Badge, Avatar, Alert, List, basic Table, Skeleton ve Progress",
    ),
  );
  assert.ok(
    normalizedStartHere.includes(
      "`M10A-T10` `DONE`: project/surface yaşam döngüsü, T02 zarfını generation-fenced saklama",
    ),
  );
  assert.match(
    normalizedStartHere,
  ...
  assert.match(
    normalizedStartHere,
    /`M10A-T14` `DONE`: immutable undo\/redo, atomik duplicate\/copy\/paste ve kimlik\/referans eşlemesi için .*?geçti\./u,
  );
  assert.match(
    normalizedStartHere,
  ...
  assert.match(
    normalizedStartHere,
    /`M10A-T17` `DONE`: Catalog-backed Design System explorer için .*?geçti\./u,
  );
  assert.match(
    normalizedStartHere,
  ...
  assert.ok(
    normalizedStartHere.includes(
      "`M10A-T19` `DONE`: ayrı Connections çalışma alanı ve dayanıklı inert taslaklar için",
    ),
  );
  assert.ok(normalizedStartHere.includes("`M10A-T21` seçilmiş sıradaki görevdir ve `NOT_STARTED`"));
  assert.equal(
...
test("pre-M11 planning inventory rejects row, dependency, count, or gate-status drift", async () => {
  ...
  for (const status of ["IN_PROGRESS", "NOT_STARTED", "BLOCKED"]) {
    const falseTaskStatus = replaceTaskBoardCell(taskBoard, "M10A-T14", 1, status);
    assert.throws(
      () => assertPreM11PlanningInventory(parseTaskBoard(falseTaskStatus)),
      /M10A-T14 must remain DONE/u,
    );
  }
});
...
test("both inventories require exact browser-proof scripts and reviewed Vite configs", async () => {
  ...
  for (const validate of [validateProofInventory, validateRepositoryWorkloadInputs]) {
    assert.equal(validate(baseline).proofCount, 123);
    ...
    const missingT07 = clone(baseline);
    missingT07.testConfigurationFiles = missingT07.testConfigurationFiles.filter(
      (file) => file !== "apps/starter-catalog-web-proof/t07-vite.config.ts",
    );
    assert.throws(() => validate(missingT07), /test-configuration file set/u);
...
test("the execution plan contains no generator, writer, shell, or changed-file shortcut", () => {
  ...
  assert.equal(steps.length, 260);
  assert.equal(steps.filter(({ id }) => id.startsWith("test-")).length, 123);
  assert.deepEqual(
...
test("the exact single-pass plan rejects command removal and duplicate root coverage", () => {
  ...
  assert.deepEqual(validateQualityGatePlan(steps), {
    stepCount: 260,
    planSha256: "256796ea63c1d192ae26d45ee415e38faf6b0900c95203f1f761cc3bcd81b465",
  });
