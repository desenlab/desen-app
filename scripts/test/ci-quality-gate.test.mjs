...
const EXPECTED_COMPLETED_IMPLEMENTATION_TASKS = 127;
...
test("task board retains its canonical inventory without narrative appendices", async () => {
  ...
  assert.equal(completedM10ATasks, 6);
  assert.equal(completionPercent, 72);
  assert.equal(m10aCompletionPercent, 21);
  ...
  assert.equal(statuses.get("M10A-T07"), "IN_PROGRESS");
  ...
  assert.ok(
    normalizedProjectStatus.includes(
      "M10A-T01 through M10A-T06 are DONE. M10A-T07 is `IN_PROGRESS`",
    ),
  );
