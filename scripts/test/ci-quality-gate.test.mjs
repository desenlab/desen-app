...
const EXPECTED_COMPLETED_IMPLEMENTATION_TASKS = 138;
...
test("task board retains its canonical inventory without narrative appendices", async () => {
  ...
  assert.equal(completedM10ATasks, 17);
  assert.equal(completionPercent, 78);
  assert.equal(m10aCompletionPercent, 61);
  ...
  assert.ok(
    normalizedReadme.includes(
      "**Next task:** `M10A-T18` (`NOT_STARTED`; library management and impact)",
    ),
  );
  assert.ok(normalizedProjectStatus.includes("M10A-T01 through M10A-T17 are DONE"));
  assert.ok(
    normalizedFullProjectStatus.includes(
      "M10A-T18 — library management and impact — is the selected next task",
    ),
  );
  assert.ok(
    normalizedFullProjectStatus.includes("M10A-T19 is also dependency-ready but unselected"),
  );
  ...
  assert.ok(
    normalizedStartHere.includes(
      "T18 seçilmiş sıradaki görevdir; T19 da bağımlılık açısından hazırdır; ikisi de `NOT_STARTED`.",
    ),
  );
  assert.equal(
