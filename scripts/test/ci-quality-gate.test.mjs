...
const EXPECTED_COMPLETED_IMPLEMENTATION_TASKS = 122;
...
function assertPreM11PlanningInventory({ rows, statuses }) {
  ...
  for (const taskId of M10A_TASK_IDS) {
    ...
    const expectedStatus =
      taskId === "M10A-T01" ? "DONE" : taskId === "M10A-T02" ? "IN_PROGRESS" : "NOT_STARTED";
...
test("the current repository exactly matches the reviewed live proof inventory", async () => {
  ...
  assert.deepEqual(result, {
    ...
    legacyPrerequisiteCount: 753,
    legacyPrerequisiteSha256: "f2c2cab2a7b5166d7722cc413c1079dd7c0b9b4011c6d6049d30dba6c10d3402",
    legacyLeafInvocationCount: 4577,
    legacyLeafInvocationSha256: "6ad759ada99e7b601d05dcb64cb7c3f3a2f96c6de6ff1112721e7aee7be4cddc",
...
test("task board retains its canonical inventory without narrative appendices", async () => {
  ...
  assert.equal(completedM10ATasks, 1);
  assert.equal(completionPercent, 69);
  assert.equal(m10aCompletionPercent, 4);
  ...
  assert.ok(normalizedReadme.includes("**Active task:** `M10A-T02` (`IN_PROGRESS`)"));
  assert.ok(normalizedProjectStatus.includes("**M10A-T01 is DONE**"));
  assert.ok(normalizedProjectStatus.includes("**M10A-T02 is IN_PROGRESS**"));
...
test("pre-M11 planning inventory rejects row, dependency, count, or gate-status drift", async () => {
  ...
  for (const status of ["DONE", "NOT_STARTED"]) {
    ...
    assert.throws(
      () => assertPreM11PlanningInventory(parseTaskBoard(falseTaskStatus)),
      /M10A-T02 must remain IN_PROGRESS/u,
    );
  }
