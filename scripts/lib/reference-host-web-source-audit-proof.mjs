...
export async function buildReferenceHostWebSourceAuditEvidence(rawOptions = undefined) {
  ...
  const authority = await readCheckpointedFrozenArtifact("M05-T09");
  if (
    ...
    fail(
      "REFERENCE_HOST_SOURCE_AUDIT_ARTIFACT_DRIFT",
      "The checkpoint-authenticated M05-T09 artifact identity drifted.",
    );
  }
  const artifactBytes =
...
/**
 * Runs the live semantic/source audit after authenticating this reader through the central
 * append-only checkpoint. Successor-reader authority is intentionally not reconstructed here.
 */
export async function verifyCurrentReferenceHostWebSourceAuditEvidence(rawOptions = undefined) {
  const options = captureOptions(rawOptions, ["workspaceRoot"], "current audit verify");
  const workspaceRoot = await resolveWorkspaceRoot(
    optionalString(options.workspaceRoot, "workspaceRoot"),
  );
  const checkpoint = await verifyProofReaderCheckpoints({ workspaceRoot });
  const current = await buildCurrentReferenceHostWebSourceAuditEvidence({ workspaceRoot });
  return Object.freeze({
    result: "PASS",
    checkpoint,
