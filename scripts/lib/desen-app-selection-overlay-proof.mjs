...
const M10_USER_CREATED_BLANK_PROJECT_REVIEWED_README_RECEIPT = Object.freeze({
  ...
  bytes: 43_576,
  sha256: "0273d175de197c7f2171e1771979d131b848f19b670769cd66bbd9040d278e46",
});
...
function assertNoPrivateInspection(sourceFile, relativePath, allowApplicationNavigation = false) {
  const privateProperties = collectDescendants(sourceFile, ts.isPropertyAccessExpression).filter(
    (access) => PRIVATE_DOM_PROPERTIES.includes(access.name.text),
  );
  function isExactAuthoringLayerRowLookup(access) {
    if (
      access.getText(sourceFile) !== "child.querySelector" ||
      !ts.isCallExpression(access.parent) ||
      access.parent.expression !== access ||
      access.parent.arguments.length !== 1 ||
      !ts.isStringLiteral(access.parent.arguments[0]) ||
      access.parent.arguments[0].text !== "[data-layer-drop-row-node-id]" ||
    ...
    while (ancestor !== undefined) {
      if (ts.isFunctionDeclaration(ancestor)) {
        return ancestor.name?.text === "projectNearestDrop";
      }
  ...
  const allowed = allowApplicationNavigation
    ? privateProperties.filter(isAllowedApplicationProperty)
    : [];
  const forbiddenProperties = allowApplicationNavigation
    ? privateProperties.filter((access) => !isAllowedApplicationProperty(access))
    : privateProperties;
  ...
  if (
    ...
    fail("PRIVATE_STRUCTURE_AUTHORITY", `${relativePath} admits private DOM or React authority.`, {
      properties: forbiddenProperties.map((access) => access.getText(sourceFile)),
...
function inspectApplicationSource(rawSource) {
  ...
  const imports = importModules(sourceFile);
  const editorCoreImports = imports.filter(({ module }) => module === "@desen/editor-core");
  ...
  if (
    ...
    imports.some(
      ({ module }) =>
        module === "@desen/editor-web" ||
        module.includes("control-plane") ||
        module.startsWith("@desen/editor-core/"),
    )
  ) {
  ...
  if (stateCalls.length !== 9) {
    fail(
      "SOURCE_POLICY_VIOLATION",
      "SurfaceEditor must own the two M10 mount-snapshot states plus route-local mode, selection, transient diagnostics, diagnostic selection, authoring-session, scenario, and no-port dirty-projection state.",
    );
  }
...
function inspectPackages(files) {
  ...
  if (
    ...
    app.scripts?.lint !== "eslint src test dev --max-warnings=0" ||
    ...
    [
      ...
      "@desen/protocol",
      "@desen/publisher",
    ].some((dependency) => app.dependencies?.[dependency] !== "workspace:*") ||
    app.devDependencies?.["@desen/editor-web"] !== undefined
  ) {
  ...
  return deepFreeze({
    ...
    devCommand: app.scripts.dev,
    lintCommand: app.scripts.lint,
    typecheckCommand: app.scripts.typecheck,
    localRuntimeTestCommand: app.scripts["test:local-runtime"],
    productBootstrapTestCommand: app.scripts["test:product-bootstrap"],
...
function authenticateNamedSlotArtifact(bytes, files) {
  ...
  const receiptsByPath = reviewedSuccessorReceiptMap(trackedReceipts, files);
