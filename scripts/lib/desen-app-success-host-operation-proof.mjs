...
/** Historical M10-T03 identity; current archive bytes require the separate AR-01 transport pin. */
...
function authenticateHistoricalReaderBridge(compressedBytes, parentArtifact) {
  const historicalPin = DESEN_APP_T03_HISTORICAL_READER_BRIDGE_PIN;
  let transport;
  try {
  ...
  for (const receipt of expectedReceipts) {
    ...
    if (
      (bytes.byteLength !== receipt.bytes || sha256(bytes) !== receipt.sha256) &&
      (receipt.path !==
...
async function preserveHistoricalEvidence(currentArtifact, files, workspaceRoot) {
  ...
  const transport = getHistoricalArchiveRedactionPin(T03_HISTORICAL_READER_BRIDGE_PATH);
  ...
  for (let index = 0; index < historicalReceipts.length; index += 1) {
    ...
    if (
      ...
      (historical.path !== T03_HISTORICAL_READER_BRIDGE_PATH &&
...
/**
 * Freshly verifies the retained M10-T04 technical projection and its amended archive transport.
 *
...
/** Verifies frozen M10-T04 technical evidence and separately reports current AR-01 receipts. */
...
/** Materializes retained T03 technical bytes and amended archives before caller mutations. */
...
/** Returns a defensive copy of retained M10-T03 technical or amended archive bytes. */
