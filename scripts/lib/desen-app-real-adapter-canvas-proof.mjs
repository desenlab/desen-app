...
const CURRENT_APP_SOURCE_PATHS = Object.freeze(
  [
    ...
    ...M10_USER_CREATED_BLANK_PROJECT_SOURCE_PATHS,
...
const CURRENT_APP_TYPESCRIPT_SOURCE_PATHS = Object.freeze(
  CURRENT_APP_SOURCE_PATHS.filter(
    (relativePath) =>
      ...
      !M10_USER_CREATED_BLANK_PROJECT_SOURCE_PATHS.includes(relativePath) &&
      !M10_VISUAL_BEHAVIOR_AUTHORING_RECEIPT_PATHS.includes(relativePath),
  ),
);
...
const ALLOWED_RUNTIME_PACKAGE_EDGES = Object.freeze({
  ...
  "editor-web": Object.freeze(["editor-core", "editor-web"]),
...
// T17 adds the catalog-backed design-system route and its source projection. Preserve the
// reviewed T15 projection while admitting the exact current successor graph.
const M10A_T17_CURRENT_RUNTIME_RESOLUTION = Object.freeze({
  ...M10A_T15_CURRENT_RUNTIME_RESOLUTION,
  moduleCount: 709,
  staticEdges: 3_047,
...
function inspectPackage(appBytes, rootBytes) {
  ...
  if (
    ...
    manifest.devDependencies?.["@desen/editor-web"] !== undefined ||
  ...
  return deepFreeze({
    ...
    localRuntimeFocusedTest: manifest.scripts["test:local-runtime"],
    productBootstrapFocusedTest: manifest.scripts["test:product-bootstrap"],
...
export async function buildDesenAppRealAdapterCanvasEvidence(rawOptions = undefined) {
  ...
  const namedSlotEvidence = authenticateNamedSlotArtifact(
