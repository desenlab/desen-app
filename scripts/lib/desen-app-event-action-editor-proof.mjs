...
function authenticateM10EmptyProjectBrowserE2eSuccessor(files) {
  ...
  for (const expected of [
    ...
    if (
      ...
      (!M10_USER_CREATED_BLANK_PROJECT_TRACKED_PATHS.includes(expected.path) &&
...
const RETAINED_HISTORICAL_PATHS = Object.freeze(
  TRACKED_PATHS.filter(
    (relativePath) =>
      ...
      !M10_USER_CREATED_BLANK_PROJECT_TRACKED_PATHS.includes(relativePath),
  ),
);
...
function inspectCssSource(source) {
  ...
  return deepFreeze({
    ...
    stableCompactLayerGapPresentation: true,
    ...
    dedicatedDragHandlesAndSeparateAddPresentation: true,
...
function authenticatePublishActivationSuccessor(files) {
  ...
  for (const relativePath of T14_SUCCESSOR_RECEIPT_PATHS) {
    if (M10_USER_CREATED_BLANK_PROJECT_TRACKED_PATHS.includes(relativePath)) continue;
    const receipt = receiptMap.get(relativePath);
    const bytes = files.get(relativePath);
...
export async function buildDesenAppEventActionEditorEvidence(rawOptions = undefined) {
  ...
  const currentCompatibility = deepFreeze({
    ...
    retainedAuthoringUx: {
      ...
      stableCompactLayerGaps: source.css.stableCompactLayerGapPresentation,
      ...
      releaseDriftRetainsLastAcceptedProjection:
        source.application.releaseDriftRetainsLastAcceptedProjection,
      nestedSlotSurfaceOwnsDropEvents: source.application.nestedSlotSurfaceOwnsDropEvents,
      explicitNoOpPlacementFeedback: source.application.explicitNoOpPlacementFeedback,
      componentDragAuthorityLimitedToDedicatedHandle:
        source.application.componentDragAuthorityLimitedToDedicatedHandle,
      dedicatedLayerDragHandle: source.application.dedicatedLayerDragHandle,
      componentPanelWideDropSurface: source.application.componentPanelWideDropSurface,
      stickyComponentTargetDirectDropSurface:
        source.application.stickyComponentTargetDirectDropSurface,
