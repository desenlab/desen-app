...
const CONTINUOUS_VALIDATION_DIST_PATHS = Object.freeze([
...
const EXPECTED_T08_PUBLIC_TEST_NAMES = Object.freeze([
...
const CURRENT_COMPATIBILITY_PATHS = Object.freeze([
  ...
  ...PERSISTENCE_DIST_PATHS,
  CONTINUOUS_VALIDATION_SOURCE_PATH,
  ...CONTINUOUS_VALIDATION_DIST_PATHS,
  AUTHORING_ROUND_TRIP_TEST_PATH,
  AUTHORING_ROUND_TRIP_TYPES_PATH,
...
function verifyBoundary(files) {
  ...
  const eventActionEditsSource = decodeUtf8(
    files.get(EVENT_ACTION_EDITS_SOURCE_PATH),
    EVENT_ACTION_EDITS_SOURCE_PATH,
  );
  const eventActionSourceExports = exportedNames(eventActionEditsSource);
  exactArray(
    eventActionSourceExports.names,
    [
      ...EXPECTED_EVENT_ACTION_EDIT_RUNTIME_EXPORTS,
      ...EXPECTED_EVENT_ACTION_EDIT_TYPE_EXPORTS,
    ].sort(compareText),
    "SOURCE_DRIFT",
    "Event/action-edit exports",
  );
  if (
    eventActionSourceExports.tsdocDeclarations !==
  ...
  const persistenceSource = decodeUtf8(files.get(PERSISTENCE_SOURCE_PATH), PERSISTENCE_SOURCE_PATH);
  const persistenceSourceExports = exportedNames(persistenceSource);
  exactArray(
    persistenceSourceExports.names,
    [...EXPECTED_PERSISTENCE_RUNTIME_EXPORTS, ...EXPECTED_PERSISTENCE_TYPE_EXPORTS].sort(
      compareText,
    ),
    "SOURCE_DRIFT",
    "Persistence exports",
  ...
  if (
    persistenceSourceExports.tsdocDeclarations !==
  ...
  const continuousValidationSourceExports = exportedNames(continuousValidationSource);
  exactArray(
    continuousValidationSourceExports.names,
    [
      ...EXPECTED_CONTINUOUS_VALIDATION_RUNTIME_EXPORTS,
      ...EXPECTED_CONTINUOUS_VALIDATION_TYPE_EXPORTS,
    ].sort(compareText),
    "SOURCE_DRIFT",
    "Continuous-validation exports",
  );
  if (
    continuousValidationSourceExports.tsdocDeclarations !==
  ...
  const distEventActionEdits = decodeUtf8(
    files.get("packages/editor-core/dist/event-action-edits.js"),
    "packages/editor-core/dist/event-action-edits.js",
  );
  const distEventActionEditsDeclaration = decodeUtf8(
    files.get("packages/editor-core/dist/event-action-edits.d.ts"),
    "packages/editor-core/dist/event-action-edits.d.ts",
  );
  const distPersistence = decodeUtf8(
    files.get("packages/editor-core/dist/persistence.js"),
    "packages/editor-core/dist/persistence.js",
  );
  const distPersistenceDeclaration = decodeUtf8(
    files.get("packages/editor-core/dist/persistence.d.ts"),
    "packages/editor-core/dist/persistence.d.ts",
  );
  const distContinuousValidation = decodeUtf8(
    files.get("packages/editor-core/dist/continuous-validation.js"),
    "packages/editor-core/dist/continuous-validation.js",
  );
  const distContinuousValidationDeclaration = decodeUtf8(
    files.get("packages/editor-core/dist/continuous-validation.d.ts"),
    "packages/editor-core/dist/continuous-validation.d.ts",
  );
  ...
  exactArray(
    staticModuleSpecifiers(distEventActionEdits),
    ["@desen/protocol", "./source-document.js"],
    "EMITTED_DRIFT",
    "Emitted event/action-edit edges",
  );
  const eventActionDeclarationExports = exportedNames(distEventActionEditsDeclaration);
  exactArray(
    eventActionDeclarationExports.names,
    [
      ...EXPECTED_EVENT_ACTION_EDIT_RUNTIME_EXPORTS,
      ...EXPECTED_EVENT_ACTION_EDIT_TYPE_EXPORTS,
    ].sort(compareText),
    "EMITTED_DRIFT",
    "Emitted event/action-edit declarations",
  );
  if (
    eventActionDeclarationExports.tsdocDeclarations !==
    EXPECTED_EVENT_ACTION_EDIT_RUNTIME_EXPORTS.length +
      EXPECTED_EVENT_ACTION_EDIT_TYPE_EXPORTS.length
  ) {
    fail("TSDOC_DRIFT", "Emitted event/action-edit declarations lost TSDoc.");
  }
  exactArray(
    staticModuleSpecifiers(distPersistence),
    ["@desen/protocol", "./source-document.js"],
    "EMITTED_DRIFT",
    "Emitted persistence edges",
  );
  const persistenceDeclarationExports = exportedNames(distPersistenceDeclaration);
  exactArray(
    persistenceDeclarationExports.names,
    [...EXPECTED_PERSISTENCE_RUNTIME_EXPORTS, ...EXPECTED_PERSISTENCE_TYPE_EXPORTS].sort(
      compareText,
    ),
    "EMITTED_DRIFT",
  ...
  if (
    persistenceDeclarationExports.tsdocDeclarations !==
    EXPECTED_PERSISTENCE_RUNTIME_EXPORTS.length + EXPECTED_PERSISTENCE_TYPE_EXPORTS.length
  ) {
    fail("TSDOC_DRIFT", "Emitted persistence declarations lost TSDoc.");
  }
  exactArray(
    staticModuleSpecifiers(distContinuousValidation),
    ["@desen/protocol", "@desen/validator", "./source-document.js"],
    "EMITTED_DRIFT",
    "Emitted continuous-validation edges",
  );
  const continuousValidationDeclarationExports = exportedNames(distContinuousValidationDeclaration);
  exactArray(
    continuousValidationDeclarationExports.names,
    [
      ...EXPECTED_CONTINUOUS_VALIDATION_RUNTIME_EXPORTS,
      ...EXPECTED_CONTINUOUS_VALIDATION_TYPE_EXPORTS,
    ].sort(compareText),
  ...
  if (
    ...
    EXPECTED_T08_PUBLIC_TEST_NAMES.some((name) => !publicTestNames.includes(name))
  ) {
  ...
  if (countTypeAssertions(publicTypes) !== 102) {
    fail(
      "TEST_INVENTORY_DRIFT",
      "The public compiler-negative inventory must remain one hundred two.",
    );
  }
...
/** Captures own-data options and rebuilds M08-T02 from freshly authenticated inputs. */
...
async function buildCapturedEvidence(options) {
  ...
  const currentCompatibility = deepFreeze({
    ...
    packageBoundary: {
      ...
      additiveEmittedFiles:
        STRUCTURAL_EDITS_DIST_PATHS.length +
        CONTENT_EDITS_DIST_PATHS.length +
        STATE_BINDING_EDITS_DIST_PATHS.length +
        EVENT_ACTION_EDITS_DIST_PATHS.length +
        PERSISTENCE_DIST_PATHS.length +
        CONTINUOUS_VALIDATION_DIST_PATHS.length,
...
export async function verifyEditorCoreStableIdInsertEvidence(rawOptions = undefined) {
  ...
  readProofDocumentPin(decodeUtf8(preflightProofBytes, "M08-T02 proof document"));
