...
const APP_ORIGIN = "http://127.0.0.1:4188";
const PACKAGE_ROOT = import.meta.dirname;
const configuredProofTemp = process.env.DESEN_M10A_T03_PROOF_TEMP;
const proofTemp = configuredProofTemp ?? mkdtempSync(join(tmpdir(), "desen-m10a-t03-"));

if (!isAbsolute(proofTemp) || resolve(proofTemp) !== proofTemp) {
  throw new TypeError("DESEN_M10A_T03_PROOF_TEMP must be an absolute runner-owned temp directory.");
}
...
export default defineConfig({
  ...
  testMatch: "design-system-workbench.pw.ts",
  ...
  use: {
    ...
    video: "off",
    viewport: { height: 920, width: 1_440 },
  },
