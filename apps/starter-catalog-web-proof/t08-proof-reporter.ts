...
import {
  STARTER_T08_BROWSER_PROOF_ASSERTION_NAMES,
  STARTER_T08_BROWSER_PROOF_ASSERTION_OWNERS,
  STARTER_T08_BROWSER_PROOF_TEST_TITLES,
} from "./t08-proof-contract.js";
...
const expectedTitles: readonly string[] = Object.freeze(
  Object.values(STARTER_T08_BROWSER_PROOF_TEST_TITLES).sort(),
);

/** Emits a bounded M10A-T08 claim receipt from exact Playwright case completions. */
export default class StarterT08ProofReporter implements Reporter {
  readonly #outputFile: string;
  ...
  public onEnd(result: FullResult): void {
    ...
    const assertions = Object.fromEntries(
      STARTER_T08_BROWSER_PROOF_ASSERTION_NAMES.map((name) => [
        name,
        this.#results.get(STARTER_T08_BROWSER_PROOF_ASSERTION_OWNERS[name]) === "PASS",
      ]),
    );
