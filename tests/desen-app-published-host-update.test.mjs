...
const M10A_T03_LOCKFILE_RECEIPT = Object.freeze({
  bytes: 139_714,
  sha256: "073a9dc3f6ce05d8e7672a74eea9296b4fee7ed8db9858cdd90731beed7ec74a",
});
const M10A_T04_LOCKFILE_RECEIPT = Object.freeze({
  bytes: 140_115,
...
const M10A_T03_T02_INPUT_RECEIPTS = Object.freeze([
  Object.freeze({
    path: "dependency-cruiser.config.cjs",
    bytes: 16_776,
...
const M10A_T04_T03_INPUT_RECEIPTS = Object.freeze([
  Object.freeze({
    path: "dependency-cruiser.config.cjs",
    bytes: 16_861,
...
function projectM10AT01Lockfile(m10aT02Lockfile) {
  return projectLockfileEntries(m10aT02Lockfile, M10A_T02_LOCKFILE_ADDED_ENTRIES);
}
...
test(DESEN_APP_PUBLISHED_HOST_UPDATE_ROOT_TEST_NAMES[4], async () => {
  ...
  assert.equal(current.appSourceAudit.completeSourceFiles, 80);
  assert.equal(current.appSourceAudit.productionGraphSourceFiles, 75);
  assert.equal(current.runtimeResolution.app.moduleCount, 710);
  assert.equal(current.runtimeResolution.app.staticEdges, 3_049);
  ...
  assert.equal(appEntryOutput?.bytes, 3_707_567);
  ...
  const typeOnlyPath = "apps/desen-app/src/authoring-persistence-types.ts";
  assert.ok(!current.runtimeResolution.appModules.some(({ id }) => id === typeOnlyPath));
  for (const addedPath of [...t15RuntimePaths, typeOnlyPath]) {
    assert.ok(current.appSourceAudit.inventory.includes(addedPath));
    if (addedPath !== typeOnlyPath)
      assert.ok(current.runtimeResolution.appModules.some(({ id }) => id === addedPath));
    const mutation = structuredClone(current);
    const receipt = mutation.appSourceAudit.sourceReceipts.find(({ path }) => path === addedPath);
    assert.ok(receipt);
    receipt.sha256 = `sha256:${"0".repeat(64)}`;
    assert.throws(
      () => projectM10AT12CurrentGraphAudit(mutation, t08Artifact.authority.currentGraphAudit),
      expectedError("SUCCESSOR_POLICY_VIOLATION"),
    );
  }
  const missingT15Module = structuredClone(current);
  missingT15Module.runtimeResolution.appModules =
    missingT15Module.runtimeResolution.appModules.filter(
      ({ id }) => id !== "apps/desen-app/src/project-authoring-controller.ts",
    );
  assert.throws(
    () =>
      projectM10AT12CurrentGraphAudit(missingT15Module, t08Artifact.authority.currentGraphAudit),
...
test(DESEN_APP_PUBLISHED_HOST_UPDATE_ROOT_TEST_NAMES[6], async () => {
  ...
  const t15Command = "playwright test --config t15-playwright.config.ts";
  const suite = parsedBrowserPackage.scripts["test:e2e"];
  assert.equal(suite.endsWith(` && ${t12Command} && ${t15Command}`), true);
  const invalidPackages = [
    ...
    { command: suite.replace(` && ${t15Command}`, ""), code: "SUCCESSOR_POLICY_VIOLATION" },
    {
      command: `${t15Command} && ${suite.replace(` && ${t15Command}`, "")}`,
      code: "SUCCESSOR_POLICY_VIOLATION",
    },
    { command: `${suite} && ${t15Command}`, code: "SUCCESSOR_POLICY_VIOLATION" },
  ];
...
test(DESEN_APP_PUBLISHED_HOST_UPDATE_ROOT_TEST_NAMES[8], async () => {
  ...
  assert.deepEqual(compatibility.t04LockfileSuccessor, {
    ...
    additivePredecessor: {
      authority: "M10A-T03",
      ...M10A_T03_LOCKFILE_RECEIPT,
    },
  });
  assert.deepEqual(compatibility.t03LockfileSuccessor, {
    task: "M10A-T03",
    ...M10A_T03_LOCKFILE_RECEIPT,
    additivePredecessor: {
      authority: "M10A-T02",
...
test(DESEN_APP_PUBLISHED_HOST_UPDATE_ROOT_TEST_NAMES[9], async () => {
  for (const receipt of M10A_T15_INPUT_RECEIPTS) {
    ...
    const expectedCurrent =
      t18Successor?.current ?? t17Successor?.current ?? t16Successor?.current ?? receipt.current;
    ...
    assert.equal(predecessor.byteLength, receipt.predecessor.bytes, receipt.path);
    assert.equal(
      createHash("sha256").update(predecessor).digest("hex"),
      receipt.predecessor.sha256,
      receipt.path,
    );
    const mutated = Buffer.from(current);
    mutated[Math.floor(mutated.length / 2)] ^= 1;
    for (const invalid of [mutated, current.subarray(1), predecessor]) {
      assert.throws(
        () => projectM10AT15HistoricalInput(receipt.path, invalid),
        expectedError("SUCCESSOR_POLICY_VIOLATION"),
        receipt.path,
      );
    }
  ...
  const t14BrowserPackage = projectM10AT15HistoricalInput(
    "apps/desen-app-browser-e2e/package.json",
    liveBrowserPackage,
  );
  assert.equal(t14BrowserPackage.byteLength, M10A_T12_BROWSER_PACKAGE_RECEIPT.bytes);
  assert.equal(
    createHash("sha256").update(t14BrowserPackage).digest("hex"),
  ...
  for (const { relativePath, currentReceipt, predecessorReceipt } of [
    ...
    const t14Bytes = projectM10AT15HistoricalInput(relativePath, currentBytes);
    assert.equal(t14Bytes.byteLength, currentReceipt.bytes, relativePath);
    assert.equal(
      createHash("sha256").update(t14Bytes).digest("hex"),
  ...
  const m10aT01Lockfile = projectM10AT01Lockfile(m10aT02Lockfile);
  ...
  assert.deepEqual(
    authenticateM10AT02LockfileSuccessor(m10aT02Lockfile).predecessorBytes,
    m10aT01Lockfile,
  );
  ...
  const t03SuccessorMutations = Object.freeze([
    ...
    Object.freeze(["missing", m10aT02Lockfile]),
    Object.freeze([
      "reordered",
      Buffer.from(
        replaceOnce(
          liveLockfileText,
          t03DependencyOrder.join(""),
          [...t03DependencyOrder].reverse().join(""),
        ),
      ),
    ]),
  ]);
  for (const [mutation, rejectedLockfile] of t03SuccessorMutations) {
  ...
  const t02SuccessorMutations = Object.freeze([
    Object.freeze(["tampered", changedByte(m10aT02Lockfile)]),
    Object.freeze([
      "extra",
      Buffer.from(
        replaceOnce(
          m10aT02Lockfile.toString("utf8"),
  ...
  for (const [mutation, rejectedLockfile] of t02SuccessorMutations) {
    assert.notDeepEqual(rejectedLockfile, m10aT02Lockfile, mutation);
    await assert.rejects(
      buildDesenAppPublishedHostUpdateEvidence({
        fileOverrides: new Map([["pnpm-lock.yaml", rejectedLockfile]]),
      }),
