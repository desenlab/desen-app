...
test("anchors the current M06-T02 reader and exact root successor in evidence", async () => {
  const built = await buildPublisherCatalogPinningEvidence();
  ...
  const rootRelativePath = "tests/publisher-catalog-pinning.test.mjs";
  const currentRoot = await readFile(new URL(`../${rootRelativePath}`, import.meta.url));
  await assert.rejects(
    buildPublisherCatalogPinningEvidence({
      trackedFileBytes: {
        [rootRelativePath]: Buffer.concat([currentRoot, Buffer.from("// unreviewed successor\n")]),
      },
    }),
    hasCode("PUBLISHER_CATALOG_PINNING_COMPATIBILITY_DRIFT"),
  );
});
