...
import {
  ...
  M10_GATE_M10A_T11_CURRENT_HOST_AUDIT as T11_CURRENT_HOST_AUDIT,
...
test("G10 current reader admits only the exact M10A-T01, isolated M10A-T10, or reachable M10A-T11 graph successor", () => {
  ...
  const t11Current = structuredClone(T11_CURRENT_HOST_AUDIT);
  assert.deepEqual(projectHistoricalHostAudit(t11Current), historical);
  assert.deepEqual(t11Current, T11_CURRENT_HOST_AUDIT);
  ...
  for (const key of [
    ...
    const changed = structuredClone(CURRENT_HOST_AUDIT);
  ...
  const unreviewedReachableGraph = structuredClone(T11_CURRENT_HOST_AUDIT);
  unreviewedReachableGraph.appModules += 1;
  assert.throws(
