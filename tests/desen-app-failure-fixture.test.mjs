...
test(DESEN_APP_FAILURE_FIXTURE_ROOT_TEST_NAMES[8], async () => {
  ...
  const transport = getHistoricalArchiveRedactionPin(
    DESEN_APP_T02_HISTORICAL_READER_BRIDGE_PIN.path,
  );
  ...
  const noOp = await buildDesenAppFailureFixtureEvidence({
    fileOverrides: new Map([
      [
        DESEN_APP_T02_HISTORICAL_READER_BRIDGE_PIN.path,
        await readFile(path.join(ROOT, DESEN_APP_T02_HISTORICAL_READER_BRIDGE_PIN.path)),
      ],
    ]),
  });
