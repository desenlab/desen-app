...
import {
  EDITOR_CORE_CONTENT_EDITS_PREREQUISITE_PINS,
  EDITOR_CORE_CONTENT_EDITS_ROOT_TEST_NAMES,
  EditorCoreContentEditsProofError,
  buildEditorCoreContentEditsEvidence,
  verifyEditorCoreContentEditsEvidence,
  writeEditorCoreContentEditsEvidence,
} from "../scripts/lib/editor-core-content-edits-proof.mjs";

const ROOT = path.resolve(import.meta.dirname, "..");
const CONTENT_SOURCE = "packages/editor-core/src/content-edits.ts";
...
const CONTENT_DIAGNOSTIC_CODES = Object.freeze([
...
function expectedError(code) {
  return (error) => error instanceof EditorCoreContentEditsProofError && error.code === code;
}
...
function prerequisiteOption(task, suffix) {
  const prefix = task === "M08-T02" ? "t02" : "t03";
  return `${prefix}Prerequisite${suffix}`;
}

before(async () => {
  built = await buildEditorCoreContentEditsEvidence();
});
...
test("[authority] authenticates exact frozen M08-T02/T03 artifacts and the isolated runtime graph", async () => {
  assert.equal(built.artifact.schemaVersion, 1);
  assert.equal(built.artifact.proofId, "editor-core-content-edits");
  assert.equal(built.artifact.profile, "desen.editor-core.content-edits-proof.v1");
  assert.equal(built.artifact.task, "M08-T04");
  ...
  assert.equal(built.artifact.publicApi.contentPublicDeclarations, 36);
  assert.equal(built.artifact.publicApi.contentTsdocDeclarations, 36);
  assert.deepEqual(built.artifact.executionAuthority, {
    ...
    runtimeFiles: 27,
    editorFiles: 6,
    retainedPredecessorEditorFiles: 3,
    dependencyFiles: 21,
    dependencyModules: 19,
    dependencyManifests: 2,
    prerequisites: EDITOR_CORE_CONTENT_EDITS_PREREQUISITE_PINS.map((pin) => ({
      task: pin.task,
      path: pin.path,
      sha256: pin.sha256,
    })),
  ...
  assert.equal(built.artifact.executionAuthority.editorReceipts.length, 6);
  assert.equal(built.artifact.executionAuthority.dependencyReceipts.length, 21);

  const prerequisiteArtifacts = await Promise.all(
    EDITOR_CORE_CONTENT_EDITS_PREREQUISITE_PINS.map(async (pin) =>
      JSON.parse(await readFile(path.join(ROOT, pin.path), "utf8")),
    ),
  );
  const t03Artifact = prerequisiteArtifacts[1];
  assert.deepEqual(
    built.artifact.executionAuthority.dependencyReceipts,
    t03Artifact.executionAuthority.dependencyReceipts,
  );
  const retainedEditorReceipts = new Map(
    t03Artifact.executionAuthority.editorReceipts.map((receipt) => [receipt.path, receipt]),
  );
  for (const receipt of built.artifact.executionAuthority.editorReceipts.filter(({ path: name }) =>
    ["source-document.js", "stable-id-insert.js", "structural-edits.js"].some((fileName) =>
  ...
  assert.equal(built.currentCompatibility.frozenAuthority.retainedTaskTimeReceipts, 52);
...
test("[determinism] two fresh M08-T04 builds are byte-identical", async () => {
  const first = await buildEditorCoreContentEditsEvidence();
  const second = await buildEditorCoreContentEditsEvidence();
  assert.deepEqual(first.artifactBytes, second.artifactBytes);
  assert.equal(first.artifactSha256, second.artifactSha256);
});

test("[behavior] proves all fourteen content commands, stable identity, limits, and atomic diagnostics", () => {
  ...
  assert.deepEqual(built.artifact.behavior.diagnostics, {
    editorCodes: CONTENT_DIAGNOSTIC_CODES,
...
test("[mutation] rejects runtime substitution and tracked boundary mutation", async () => {
  ...
  const runtime = Object.fromEntries(
    ["createDesenEditorDocument", ...CONTENT_RUNTIME_EXPORTS].map((name) => [
      name,
      (...args) => {
  ...
  await assert.rejects(
    buildEditorCoreContentEditsEvidence({ runtime }),
    expectedError("RUNTIME_OVERRIDE_REJECTED"),
  );
  assert.equal(runtimeExecuted, false);

  const source = await readFile(path.join(ROOT, CONTENT_SOURCE));
  ...
  await assert.rejects(
    buildEditorCoreContentEditsEvidence({
      fileOverrides: { [CONTENT_SOURCE]: changedByte(source) },
    }),
...
test("[artifact] verifies exact artifact bytes and one exact final proof pin", async () => {
  ...
  const directory = await temporaryDirectory("desen-m08-t04-proof-preflight-");
  ...
  try {
    const verified = await verifyEditorCoreContentEditsEvidence({
    ...
    assert.deepEqual(
      ...
      EDITOR_CORE_CONTENT_EDITS_PREREQUISITE_PINS.map(({ sha256 }) => sha256),
    );
    ...
    const retainedProofPath = path.join(directory, "retained-proof.md");
    const originalMkdtemp = fsPromises.mkdtemp;
    let proofReplaced = false;
    const runtimeImport = test.mock.method(fsPromises, "mkdtemp", async (...arguments_) => {
      if (!proofReplaced) {
        await rename(proofDocumentPath, retainedProofPath);
        await symlink(retainedProofPath, proofDocumentPath);
        proofReplaced = true;
      }
    ...
    try {
      ...
      assert.equal(proofReplaced, true);
    } finally {
      runtimeImport.mock.restore();
      syncBuiltinESMExports();
    }
  } finally {
...
test("[writer] atomically commits exact bytes and preserves the previous destination on failure", async () => {
  const directory = await temporaryDirectory("desen-m08-t04-writer-");
  ...
  const result = await writeEditorCoreContentEditsEvidence({ destinationPath });
...
test("[writer-filesystem] rejects symlink, hard-link, and non-file destinations", async () => {
  const directory = await temporaryDirectory("desen-m08-t04-destination-");
  ...
  try {
    for (const destinationPath of [
      ...
      await assert.rejects(
        writeEditorCoreContentEditsEvidence({ destinationPath }),
        expectedError("FILESYSTEM_UNSAFE"),
      );
    }
    assert.equal(
  ...
  const racedHardLink = path.join(directory, "raced-hard-link.json");
  await writeFile(racedDestination, "preserved\n");
  const originalMkdtemp = fsPromises.mkdtemp;
  let destinationLinked = false;
  const runtimeImport = test.mock.method(fsPromises, "mkdtemp", async (...arguments_) => {
    if (!destinationLinked) {
      await link(racedDestination, racedHardLink);
  ...
  try {
    await assert.rejects(
      writeEditorCoreContentEditsEvidence({ destinationPath: racedDestination }),
      expectedError("FILESYSTEM_UNSAFE"),
    );
    assert.equal(destinationLinked, true);
    assert.equal(await readFile(racedDestination, "utf8"), "preserved\n");
  } finally {
    runtimeImport.mock.restore();
    syncBuiltinESMExports();
  }
...
test("[filesystem] rejects linked artifact/proof and linked, replaced, or raced prerequisites", async () => {
  const directory = await temporaryDirectory("desen-m08-t04-authority-");
  for (const pin of EDITOR_CORE_CONTENT_EDITS_PREREQUISITE_PINS) {
    ...
    await assert.rejects(
      buildEditorCoreContentEditsEvidence({
        [prerequisiteOption(pin.task, "Path")]: prerequisiteSymbolic,
      }),
      expectedError("FILESYSTEM_UNSAFE"),
    );
    ...
    await assert.rejects(
      buildEditorCoreContentEditsEvidence({
        [prerequisiteOption(pin.task, "Path")]: prerequisiteHard,
      }),
      expectedError("FILESYSTEM_UNSAFE"),
    );
    ...
    await assert.rejects(
      buildEditorCoreContentEditsEvidence({
        [prerequisiteOption(pin.task, "Path")]: swappedAuthority,
    ...
    await assert.rejects(
      buildEditorCoreContentEditsEvidence({
        [prerequisiteOption(pin.task, "Path")]: parentAuthority,
        async beforeAuthorityRecheck({ absolutePath }) {
...
test("[options] rejects unknown, accessor, inherited, symbol, proxy, and shared inputs", async () => {
  ...
  for (const name of CONTENT_RUNTIME_EXPORTS) {
  ...
  for (const options of [
    ...
    { t03PrerequisiteBytes: shared },
    { runtime: runtimeAccessor },
    { beforeAuthorityRecheck: new Proxy(() => undefined, {}) },
  ]) {
    await assert.rejects(
      buildEditorCoreContentEditsEvidence(options),
      expectedError("OPTIONS_INVALID"),
    );
  }
  await assert.rejects(
...
test("[immutability] freezes evidence and states the exact nonclaim boundary", () => {
  ...
  assert.deepEqual(built.artifact.nonclaims, [
    ...
    "M08_T05_THROUGH_T08_AUTHORING_AND_PERSISTENCE",
  ...
  assert.deepEqual(
    EDITOR_CORE_CONTENT_EDITS_ROOT_TEST_NAMES,
    EDITOR_CORE_CONTENT_EDITS_ROOT_TEST_NAMES.slice(),
  );
});
